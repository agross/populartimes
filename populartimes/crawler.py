...
def add_optional_parameters(detail_json, detail, rating, rating_n, popularity, current_popularity, time_spent):
    ...
    if "internationalPhoneNumber" in detail:
        detail_json["international_phone_number"] = detail["internationalPhoneNumber"]
...
def get_populartimes_by_detail(detail):
    address = detail["formattedAddress"] if "formattedAddress" in detail \
        else detail.get("shortFormattedAddress", "")
    ...
    detail_json = add_optional_parameters(detail_json,
                                          detail,
                                          *get_populartimes_from_search(name, address))
